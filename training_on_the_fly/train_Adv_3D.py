...
def start_main_3D(job_description):
    ...
    # initialization
    model = Adv(job_description=job_description).to(job_description['device'])
    ...
    eval_mode = eval_val.eval_val_adv_3D(job_description=job_description)
    ...
    for epoch in tqiter:
        ...
        for b in range(num_batches_per_epoch):
            ...
            cla, selection, outputs_seg = model.forward(inputs, labels[:, 1:2])

            if job_description['dataset'] == 'airway':
                outputs_seg = outputs_seg * lungs

            loss_G = criterion[1](outputs_seg.to(
                job_description['device']), labels[:, 1:2])


            if selection == 0:
                loss_D = criterion[0](cla[0, 0].to(
