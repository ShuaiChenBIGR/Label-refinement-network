...
from .LR_3D_parts import *


class LR_3D(nn.Module):
    def __init__(self, job_description, trilinear=True, skip=True, LR=True):
        super(LR_3D, self).__init__()
        self.LR = LR
        self.n_channels = 1
        if self.LR:
            self.n_channels = 2
    ...
    def forward(self, x, generated_errors, real_error, select):
        if select >= 1:
            input = torch.cat([x, generated_errors], dim=1)
        else:
            input = torch.cat([x, real_error], dim=1)
