        ...
        for i, j in enumerate(patient_for_batch):
            ...
            if self.job_description['dataset'] == 'airway':
                if self._is_generate_errors_labels:
                    ...
                    # labels = seg_temp[0] # GT
                    labels = seg_temp[2] # initial results
...
def start_main_3D(job_description):
    ...
    tr_gen = AirwayDataLoader3D(job_description['train_keys'], job_description['train_initial_keys'],
    ...
    val_gen = AirwayDataLoader3D(job_description['val_keys'], job_description['val_initial_keys'],
    ...
    for epoch in tqiter:
        ...
        for b in range(num_batches_per_epoch):
            ...
            optimizer = (
            optim.SGD(model.parameters(), lr=poly_lr(epoch, job_description['epoch'], job_description['lr'], 0.9),
                      momentum=0.99, nesterov=True, weight_decay=3e-05),)
