...
from src.py.utils.vis_loss import history_log, visualize_GAN_loss
from src.py.utils.visualize_batch import visualize_GAN
...
def add_random_error(seg, lower_bound_ratio, upper_bound_ratio, error_size):
    ...
    seg_error = modify_patches(seg, error_size, lambda x: 0, ratio)
...
class AirwayDataLoader3D(DataLoader):
    ...
    def create_data_generate_errors_labels(self):
        ...
        reference_keys_procimages_file = os.path.join(self._path_basedir_info_gen_errors, 'referenceKeys_procimages.npy')
        reference_keys_nnunetimages_file = os.path.join(self._path_basedir_info_gen_errors, 'referenceKeys_nnUnetimages.npy')
...
def start_main_3D(job_description):
    ...
    dataloader_train = AirwayDataLoader3D(job_description['train_keys'], job_description['train_initial_keys'], batch_size, max_shape, 1, job_description, seed_for_shuffle=job_description['seed'], modalities=1, mode='train',
                                          is_generates_errors_labels=True,
                                          path_basedir_info_gen_errors=job_description[
                                              'Patient_DIR'])  # ADDED BY ANTONIO
    dataloader_validation = AirwayDataLoader3D(job_description['val_keys'], job_description['val_initial_keys'], batch_size, patch_size, 1, job_description, seed_for_shuffle=job_description['seed'], shuffle=False, crop='center', modalities=1, mode='valid',
                                               is_generates_errors_labels=True,
                                               path_basedir_info_gen_errors=job_description[
                                                   'Patient_DIR']
                                               )
    ...
    criterion = (nn.BCELoss(), DiceCoefficientLF())
    tqiter = trange(job_description['epoch'], desc=job_description['dataset'])
    epoch_train_initial = 0
    epoch_train_error = 0
    epoch_train_adv = 0
    epoch_train_D = 0
    epoch_val = 0
    optimizer = (optim.Adam(model.parameters(), lr=job_description['lr_G']),
                 optim.Adam(model.parameters(), lr=job_description['lr_D']))

    for epoch in tqiter:
        start_epoch = time()
        fig_loss = plt.figure(num='loss', figsize=[15, 3.8])

        # Pretrain G a few epochs:
        if epoch <= job_description['epoch_G']:
            GD = 'G'
        elif epoch <= job_description['epoch_G'] + job_description['epoch_D']:
            GD = 'D'
        else:
            # after epoch_G + epoch_D, alternate G and D for training
            if GD == 'G':
                GD = 'D'
            elif GD == 'D':
                GD = 'G'
        ...
        for b in range(num_batches_per_epoch):
            ...
            if balance > 0.3:
                while True:
                    ...
                    # length = len(torch.unique(labels))
            ...
            # if job_description['error_type'] == 'manual':
            initials_original = initials.clone()

            optimizer[0].zero_grad()
            optimizer[1].zero_grad()

            outputs_rec, cla, gt_bool = model.forward(inputs, errors, initials_original, GD, epoch, job_description)

            if job_description['dataset'] == 'airway':
                outputs_rec = outputs_rec * lungs

            # if train generator:
            if GD == 'G':
                loss_initial = criterion[1](outputs_rec[:, :], initials_original[:, :])
                loss_error = criterion[1](outputs_rec[:, :], errors[:, :])
                loss = 0.5 * loss_initial + 0.5 * loss_error
                ...
                if epoch > job_description['epoch_G'] + job_description['epoch_D']:
                    ...
                    # if fake error:
                    if gt_bool == 1:
                        loss_adv = criterion[0](cla[0].to(
                            job_description['device']), torch.from_numpy(np.array(1)).float().to(
                            job_description['device']))
                        loss += loss_adv
                        epoch_train_adv += loss_adv.item()

                loss.backward()
                optimizer[0].step()
                epoch_train_initial += loss_initial.item()
                epoch_train_error += loss_error.item()

            # if train discriminator:
            elif GD == 'D':
                # if real error:
                if gt_bool == 0:
                    loss_D = criterion[0](cla[0].to(
                        job_description['device']), torch.from_numpy(np.array(1)).float().to(
                        job_description['device']))
                # if fake error:
                elif gt_bool == 1:
                    loss_D = criterion[0](cla[0].to(
                        job_description['device']), torch.from_numpy(np.array(0)).float().to(
                        job_description['device']))
            ...
            if epoch % job_description['vis_train'] == job_description['vis_train'] - 1:
                if job_description['preview_train'] == True:
                    if b == 0:
                        ...
                        labels_original_vis = gt.cpu().detach().numpy()[0][0][slice]
                        # labels_original_vis = np.where(labels_original_vis > 0.5, 1, 0)
                        inputs_vis = errors.cpu().detach().numpy()[0][0][slice]
                        label_vis = initials.cpu().detach().numpy()[0][0][slice]
                        outputs_vis = outputs_rec.cpu().detach().numpy()[0][0][slice]
                        outputs_vis = np.where(outputs_vis > 0.5, 1, 0)

                        fig_batch = plt.figure(figsize=[7, 10])
                        visualize_GAN(labels_original_vis, inputs_vis, label_vis, outputs_vis, epoch=epoch, ratio=0)

                        if GD == 'G':
                            plt.savefig(job_description['result_path'] + '/preview_main/train/' + 'epoch_%s_G.jpg' % epoch)
                        else:
                            plt.savefig(job_description['result_path'] + '/preview_main/train/' + 'epoch_%s_D.jpg' % epoch)
                        plt.close(fig_batch)

        tqiter.set_description(job_description['dataset'] + '(train=%.4f, val=%.4f)'
                               % (epoch_train_initial, epoch_val))

        if epoch % job_description['epoch_G'] == job_description['epoch_G'] - 1:
            torch.save(model.state_dict(), job_description['result_path'] + '/model/G_%s.pth' % epoch)
        if epoch % job_description['epoch_D'] == job_description['epoch_D'] - 1:
            torch.save(model.state_dict(), job_description['result_path'] + '/model/D_%s.pth' % epoch)

        # torch.save(model.state_dict(), job_description['result_path'] + '/model/latest_G.pth')

        # save and visualize training information
        if epoch == 0:
            title = 'Epoch     G_initial     G_error     Adv     D    best_epoch\n'
            history_log(job_description['result_path'] + '/history_log.txt', title, 'w')
            history = (
                '{:3d}        {:.4f}        {:.4f}        {:.4f}       {:.4f}       {:d}\n'
                    .format(epoch, epoch_train_initial / (num_batches_per_epoch), epoch_train_error / (num_batches_per_epoch),  epoch_train_adv / (num_batches_per_epoch), epoch_train_D / (num_batches_per_epoch), best_epoch))
            ...
            visualize_GAN_loss(fig_loss, dict=dict, title=title, epoch=epoch)
